    ...
    npy_intp *unused)
{
    ...
    {
        PyErr_SetString(PyExc_ValueError, "can only add units of the same dimension");
        return -1;
    }
...
int init_add_ufunc(void)
{
    ...
    static PyType_Slot slots[] = {
        {NPY_METH_resolve_descriptors, &qq_add_resolve_descriptors},
        {NPY_METH_strided_loop, &dd_add_strided_loop},
        {0, NULL}};
    PyArrayMethod_Spec qq_AddSpec = {
        .name = "qq_add",
        .nin = 2,
        .nout = 1,
        .dtypes = dtypes,
    ...
    if (PyUFunc_AddLoopFromSpec(add, &qq_AddSpec) < 0)
    {
        Py_DECREF(add);
        return -1;
    }
