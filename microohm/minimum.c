...
static NPY_CASTING
qq_minimum_resolve_descriptors(
    PyObject *self,
...
int init_minimum_ufunc(void)
{
    PyObject *numpy = PyImport_ImportModule("numpy");
    if (numpy == NULL)
        return -1;
    PyObject *minimum = PyObject_GetAttrString(numpy, "minimum");
    Py_DECREF(numpy);
    if (minimum == NULL)
        return -1;
    static PyArray_DTypeMeta *dtypes[3] = {&QuantityDType, &QuantityDType, &QuantityDType};
    static PyType_Slot slots[] = {
        {NPY_METH_resolve_descriptors, &qq_minimum_resolve_descriptors},
        {NPY_METH_strided_loop, &dd_minimum_strided_loop},
        {0, NULL}};
    PyArrayMethod_Spec qq_MinimumSpec = {
        .name = "qq_minimum",
        .nin = 2,
        .nout = 1,
        .dtypes = dtypes,
    ...
    if (PyUFunc_AddLoopFromSpec(minimum, &qq_MinimumSpec) < 0)
    {
        Py_DECREF(minimum);
        return -1;
    }
